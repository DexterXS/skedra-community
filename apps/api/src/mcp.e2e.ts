...
if (
	...
	throw new Error(
		"MCP OAuth E2E requires MCP_E2E_BASE_URL, MCP_E2E_USER_ID, AUTH_SECRET/SKEDRA_AUTH_SECRET, and DATABASE_URL",
	);
}
...
test("remote OAuth enforces DCR limits and completes PKCE, MCP, and refresh replay handling", async () => {
	...
	const [schema, drizzle, drizzlePostgres, postgresModule] = await Promise.all([
		import("@skedra/db"),
		import("drizzle-orm"),
	...
	try {
		...
		const registrationResponse = await fetch(`${baseUrl}/api/oauth/register`, {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
		...
		assert.ok(consentToken, "authorize response must contain a consent token");
		...
		const callback = new URL(consentResponse.headers.get("location") as string);
